...
public class GMLTokenMarker extends DefaultTokenMarker
{
	...
	static KeywordSet resNames, scrNames, constructs, functions, operators, constants,
		variables;
	...
	{
		super();
		schemes.add(new BlockDescriptor("Javadoc", "/\\*(?=\\*)", "\\*/", new Color(54,
			116, 186), Font.BOLD));
		schemes.add(new BlockDescriptor("Block Comment", "/(?=\\*)", "\\*/", new Color(13,
			135, 13), Font.ITALIC));
		schemes.add(new BlockDescriptor("Doc Line Comment", "///", "$", new Color(54, 116,
			186), Font.BOLD));
		schemes.add(new BlockDescriptor("Line Comment", "//", "$", new Color(13, 135, 13),
			Font.ITALIC));
		schemes.add(new BlockDescriptor("Double Quote String", "\"", "\"", new Color(0, 0,
			100), 0));
		schemes.add(new BlockDescriptor("Single Quote String", "'", "'", new Color(0, 0,
			100), 0));
		...
		CharSymbolSet css =
			new CharSymbolSet("Operators and Separators", new Color(0, 0, 200), 0);
		char[] ca = "{[()]}!%^&*-/+=?:~<>.,;".toCharArray();
		...
		otherTokens.add(new SimpleToken("Hex literal", "\\$[0-9A-Fa-f]+", 0, new Color(100,
			100, 255)));
	}
	...
	{
		DefaultKeywords.Keyword[][] GML_KEYWORDS =
			{ GMLKeywords.CONSTRUCTS, GMLKeywords.FUNCTIONS, GMLKeywords.VARIABLES,
				GMLKeywords.OPERATORS, GMLKeywords.CONSTANTS };
		return GML_KEYWORDS;
	}
}
