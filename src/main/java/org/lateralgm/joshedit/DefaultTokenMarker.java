...
public abstract class DefaultTokenMarker implements TokenMarker {
  ...
  public static class BlockDescriptor {
    ...
    Pattern end;
    /**
     * True if this block type is allowed to span multiple lines. Can be true only if end is
     * non-null.
     */
    boolean multiline;
    /**
     * True if we can escape the ending character. If end is null, this allows us to span multiple
     * lines.
     */
    boolean escapeend;
  ...
  public static class CharSymbolSet {
     ...
     * @param markColor
     *        The font color with which to render these symbols.
     * @param font_style
     *        The font style with which to render these symbols.
     */
  ...
  public static class SimpleToken {
     ...
     * @param font_style
     *        The font style used when rendering this token.
     * @param markColor
     *        The font color with which to render this token.
     */
  ...
  static final class SchemeInfo {
     ...
     *        The type of this scheme; one of the {@link SchemeType} constants.
     * @param scheme_id
     *        The index of this scheme in its type, if applicable, or else zero.
     */
  ...
  class TokenMarkerInfoEx extends TokenMarkerInfo {
     ...
     *        The color to use.
     * @param start
     *        The start position in the current line.
     * @param end
     *        The end position in the current line.
     * @param hash
     *        Any attribute, probably in si as well, which much persist.
     * @param si
     *        More information about the scheme that created this TokenMarkerInfo.
     */
  ...
  private void mark(Code code) {
    ...
    while (invalid_line < line_count - 1) {
      ...
      if (tmi.schemeInfo.type == SchemeType.UNTERMBLOCK) {
  ...
  public void linesChanged(Code code, int start, int end) {
    ...
    if (start < invalid_line || invalid_line == -1) {
      invalid_line = start;
    }
  ...
  public ArrayList<TokenMarkerInfo> getStyles(Line jline) {
    ...
    int ischeme =
        (int) (((jline.attr & LINE_ATTRIBS.LA_SCHEMEBLOCK) >> LINE_ATTRIBS.LA_SCHEMEBITOFFSET) - 1);

    // Our function guarantees a block at the end of our code to keep the printer, well, printing.
    // Adding this blindly sometimes screws up other mechanisms. This tells us whether to do so or
    // not.
    boolean pushCapstone = true;

    int i = 0; // The position from which we will parse this thing
    FindAllBlocks: for (;;) // What we're going to do is find any and all blocks up front, and move
                            // to the end of them.
    {
      ...
      if (ischeme < 0) {
        for (int si = 0; si < schemes.size(); si++) {
          ...
          if (m.start() < mmin) { // If this one is closer to the beginning, it can potentially
                                  // consume later ones.
      ...
      for (;;) {
        Matcher mmatcher =
            schemes.get(shm).end.matcher(line.toString()).region(mminend, line.length());
        if (!mmatcher.find()) // If there's no end in sight, or that end passed our position of
                              // interest
        {
        ...
        // Now, we have found a chunk that may be the end marker, and lies before our position in
        // question.
        // Move to its end.
        i = mmatcher.end();
        ...
        {
          res.add(new TokenMarkerInfoEx(schemes.get(shm).fontStyle, schemes.get(shm).color, mmin,
              i, shm, new SchemeInfo(SchemeType.BLOCK, shm)));
        ...
        for (cp = mminend; cp < mmatcher.start(); cp++) { // So, start iterating block contents!
    ...
    for (int bi = 0; bi < res.size(); i = res.get(bi++).endPos) {
      ...
      SubschemeLoop: while (i < sp) {
        ...
        if (lookingat.lookingAt()) {
          ...
          if (!fnd) {
            if (default_kws != null) {
              res.add(bi++, new TokenMarkerInfoEx(default_kws.fontStyle, default_kws.color,
                  lookingat.start(), lookingat.end(), 0, new SchemeInfo(SchemeType.DEFKEYWORD, 0)));
            }
          }
          i = lookingat.end();
        ...
        for (int sn = 0; sn < tmChars.size(); sn++) {
          if (tmChars.get(sn).chars.contains(c)) {
            res.add(bi++, new TokenMarkerInfoEx(tmChars.get(sn).fontStyle, tmChars.get(sn).color,
                i, i + 1, 0, new SchemeInfo(SchemeType.SYMBOL, sn)));
          }
        }
        i++;
      }
