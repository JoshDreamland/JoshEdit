...
public class Selection implements Highlighter {
  ...
  /**
   * @param txt
   *        The Code from which we will make selections.
   * @param joshTxt
   *        The JoshText that owns the code.
   * @param pt
   *        The caret component of the selection.
   */
   ...
   *
   * @param str
   *        The text which would be pasted.
   ...
   *
   * @param ssel
   *        The Selection from which coordinates will be copied.
   */
  ...
  /**
   * @param c
   *        Insert a character at the cursor, erasing any selected text.
   */
  public void insert(char c) {
    switch (type) {
      ...
      case RECT:
        deleteSel();
        for (int y = Math.min(row, caret.row); y <= Math.max(row, caret.row); y++) {
          if (joshText.column_in_tab(y, caret.col)) {
            continue;
          }
  ...
  /**
   * Insert text into the current selection, replacing the current selected text if the selection is
   * not empty.
   *
   * @param str
   *        The text to insert.
  ...
  public static abstract class SortedRegion {
    ...
    /**
     * @param p1
     *        The first line-column point to sort.
     * @param p2
     *        The second line-column point to sort.
     */
  ...
  public static class SimpleHighlighter implements Highlighter {
    ...
    /**
     * @param y
     *        The row on which to highlight.
     * @param x1
     *        The first column to highlight.
     * @param x2
     *        The last column to highlight.
     */
  ...
  public static class SortedRegionHighlighter implements Highlighter {
    ...
    /**
     * @param r
     *        The SortedRegion to highlight.
     * @param type
     *        The type of the selection given by the SortedRegion.
     */
     ...
     *
     * @see org.lateralgm.joshedit.JoshText.Highlighter#paint(java.awt.Graphics, java.awt.Insets,
    ...
    public void paint(Graphics g, Insets i, CodeMetrics cm, int line_start, int line_end) {
      ...
      if (type == ST.RECT) {
        ...
        new SimpleHighlighter(r.getMinY(), r.getMinX(), r.getMaxX()).paint(g, i, cm, line_start,
            line_end);
        /*
         * int xx = line_wid_at(r.getMinY(),r.getMinX());
         * g.fillRect(i.left + xx,i.top + r.getMinY() * gh,line_wid_at(r.getMaxY(),r.getMaxX()) -
         * xx,
         * gh);
         */
  ...
  /**
   * Change the type of the current selection without affecting the start and end position on screen
   * (eg, due to tabs).
   *
   * @param t
   *        The selection type to change to.
   */
  ...
  public String getSelectedText() {
    ...
    switch (type) {
      ...
      case NORM:
        StringBuilder sbn =
            new StringBuilder(joshText.code.getsb(r.getMinY()).substring(r.getMinX()));
  ...
  interface SpecialSelectionHandler {
    /**
     * Called when the cursor position changes while selecting to allow adjustment of final
     * selection position.
     *
     * @param ss
     *        The selection metrics to adjust.
     */
  ...
  class WordSelHandler implements SpecialSelectionHandler {
    ...
    public void adjustSelection(SpecialSel ss) {
      if (type == ST.RECT? caret.col >= joshText.index_to_column(ss.irow, ss.icol)
        ...
        if (type != ST.RECT) {
          caret.col = caret.col - sp > 0 || (caret.col == ss.icol && caret.row == ss.irow)? ep : sp;
        } else {
          caret.col =
              caret.col - sp > 0 || (sp == joshText.column_to_index(ss.irow, ss.icol))? ep : sp;
        }
      } else {
        // Handle cursor
  ...
  class LineSelHandler implements SpecialSelectionHandler {
    /**
     * Upon selection change, wrap the selection to contain the entirety of the start and end
     * line(s).
     */
    @Override
    public void adjustSelection(SpecialSel ss) {
  ...
  class SpecialSel {
     ...
     *
     * @param specSelHandler
     *        The {@link SpecialSelectionHandler} to use.
     */
